<template>
    <div class='max-w-md mx-auto bg-white'>
        <div class="relative  flex items-center w-full h-12 rounded-lg focus-within:shadow-lg bg-indigo-700 overflow-hidden">
...
export default {
    ...
    methods: {
        async querieddActivities(){
            try{
                const res = await fetch(`http://localhost:8000/api/v1/activities/${this.query}`);
                if(res.ok){
